...
export function showImageCrop(element: HTMLElement, cropper: ImageCropper, state: State) {
  ...
  cropTitle.classList.add('image-editor-sidebar-settings-block-title');
  _i18n(cropTitle, 'ImageEditor.Cropper.Title');
  element.replaceChildren(cropTitle);

  const cropSettings = document.createElement('div');
  cropSettings.classList.add('image-editor-sidebar-settings-cropper-format-list');
  ...
  cropCustomSettings.classList.add('image-editor-sidebar-settings-cropper-format-compact-list');
  ...
  attachClickEvent(cropSettings, (e) => {
    const cropperFormatCell = findUpClassName(e.target, 'image-editor-sidebar-settings-cropper-format-cell');
...
function getCropperFormatCell(cropperFormat: CropperFormat, state: State) {
  ...
  btn.classList.add('btn-menu-item', 'rp-overflow', 'image-editor-sidebar-settings-cropper-format-cell');
  ...
  if(cropperFormat.needIconRotate) {
    icon.classList.add('image-editor-sidebar-settings-cropper-format-cell--rotate');
  }
  btn.append(icon);
